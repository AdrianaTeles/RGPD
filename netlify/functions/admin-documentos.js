...
export const handler = async (event) => {
  ...
  if (event.httpMethod === "PUT" && q.textos) {
    ...
    try { await setTextos(textos); } catch (err) {
    ...
    return json(200, { ok: true, textos: await getTextos() });
  }
  ...
  if (event.httpMethod === "PUT") {
    ...
    try {
      await setDocumento(nome, bytes, { ip: ipDoPedido(event) });
    } catch (err) {
    ...
    return json(200, { ok: true, documento: await getDocumentoInfo(nome) });
  }

  if (event.httpMethod === "POST" && q.acao === "repor") {
    let origem;
    try { origem = await reporDocumento(nome); } catch (err) {
      console.error("[admin-documentos] erro a repor", nome, err);
      return json(500, { message: "Erro ao repor o documento." });
    }
    console.log("[admin-documentos] reposto:", nome, "→", origem);
    await registar(event, "documento.repor", `${nome} (agora: ${origem === "bundle" ? "versão original" : "versão anterior"})`);
    return json(200, { ok: true, documento: await getDocumentoInfo(nome) });
  }
